...
void input_size(int* height, int* width) { // ввод размера лабиринта
...
bool deadend(int** maze, int height, int width) // возврат из тупика
{
    int a = 0;
...
void mazemake(int** maze, int height, int width) // создание лабиринта
{
    int c, a;
    bool b;
...
void visual(int** maze, int height, int width, RenderWindow* window, bool avto) //изображение результата с помощью SFML графики.
{
    wid = (width * 23) / 15;
    hei = (height * 25) / 10;
...
void vozvrat(int** maze, int height, int width) { // возврат от тупика до развилки 
...
void poisk_puti(int** maze, int height, int width) { // поиск пути к выходу
...
void II_proxod(int** maze, RenderWindow* window, int height, int width) { // движение бота
    stop = clock();
...
void play(int** maze, int height, int width, RenderWindow* window) { // игра 
    start = clock();
    start_pl = clock();
...
void input_time_II(RenderWindow* window) { // ввод скорости передвижения бота
...
bool mod_selectino() { // выбор режима работы пронраммы 
...
void return_menu() { // перезапуск программы 
    bool mod;
...
void vnes_lider(int time) { // внесение победителя и время его прохождения в файл и вывод данных из файла в окно windows
    ...
    text.setFillColor(Color::Blue);
    FILE* file;
    file = fopen("D:/ЛОВЗ/Лабиринт курсач/Лидеры.txt", "a");
    String string;
    char mas[10];
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (Keyboard::isKeyPressed(Keyboard::Enter)) {
                ...
                text.setFillColor(Color::Red);
                window.draw(text);
                file = fopen("D:/ЛОВЗ/Лабиринт курсач/Лидеры.txt", "r+");
